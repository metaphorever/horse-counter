...
- **1.28 Draft polish: popover quick-create + editor auto-save redesign** (2026-05-20) — Replaces hidden 0-draft popover state with inline quick-create form; adds "Add to new draft" expander for 1+-draft state; replaces explicit-save model with auto-save (immediate on stable/poem add/remove, ~5-10s debounce on drag); editor page load shows draft picker when drafts exist; UI: "Currently editing [DRAFT NAME] [Change Draft ▾]"; renamed buttons (Clear Stable / Clear Poem / Edit Details / Post Poem); clear dialogs with destination options; new `GET /me/draft/get` endpoint. Spec: `spec/phase-1.28.md`. **Overrides:** "Save Draft" button from 1.27; 0-draft hiding from PR #39 bug fix.
