...
- **Horse chip context menu in collection views** (Clover, 2026-05-23; reframed 2026-05-24) — on `/me/pasture` and `/me/saved-horses`, horse chips should have a popup context menu with actions: (1) **Send to drafts** — add this horse to a draft stable; (2) **Remove from pasture** — explicit remove, shown **only when in the pasture view** (context-sensitive); (3) **Save / Unsave** — toggle the blue-ribbon save state, available everywhere but more prominently placed on the saved-horses page. **Reframed 2026-05-24:** treat each collection page as a *dynamically composed poem* — the tap menu should be the **same chip menu** used for a horse inside a poem in the editor (`openChipMenu` in poetry.html), not a bespoke per-page popover or the current special-cased remove button. This replaces the collection-specific actions (`.collection-chip-remove` hover button) with the unified menu. Consistent with the "collections are poems" principle (memory + ROADMAP §design). Extends the 1.7 horse popover model but scoped to collection pages. Slot alongside or after the style pass. Not urgent (Clover, 2026-05-24).
