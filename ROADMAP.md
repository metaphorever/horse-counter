...
Editor UX rethink (hold — functional as-is, do not bikeshed), image-card export (interesting if pipeline is clean — needs pipeline decision first), fancy broadsheet print, horsified HTML embed, oEmbed, per-horse PQ scrape (Cloudflare-aware), real coat colors, pasture-search mode, per-horse/per-tag browse pages (nice but big infra lift — defer until FTS5 + SQLite horse dict question resolved), FTS5 search, site-popularity stats, ambient field horses, cross-posting connectors (research platform policies first — see backlog), Tumblr theme port, three-concept disambiguation pass. Follow other posters (low priority). Reply/attribution variant (build when demand signals or people start using attribution field for poem links).
...
- **Browse UI polish** (2026-05-25, verified) — two small follow-ups after live testing: (1) **tag checkboxes collapsed by category** — all categories currently expand at once; should default to collapsed/hidden by category group (click to expand), so the filter panel isn't overwhelming on first open; (2) **ratio filter as sliders** — current `<input type="number">` text boxes are unintuitive for a 0–1 ratio; replace with a dual-handle range slider or two `<input type="range">` inputs with live labels. Same session as suggested-tags bug fix.
- **Content warning tag filter on browse** (specced 2026-05-25) — opt-in exclusion of poems carrying CW-category tags, defaulting to show all. Design: (1) **UI** — separate "Hide content:" section below the existing "Show poems with:" checkboxes; only surfaces tags from `tag_categories.behavior = 'content_warning'`; unchecked = show (default), checked = exclude; visually distinct from include checkboxes so the polarity is obvious; (2) **backend** — `_browse_where` gains `excluded_slugs=()` kwarg; each excluded slug adds `AND NOT EXISTS (SELECT 1 FROM poem_tags pt JOIN tags t ON t.id = pt.tag_id WHERE t.slug = ? AND pt.status = 'approved')`; backend is fully generic (any tag can be excluded), only the UI is CW-restricted for now; (3) **URL shape** — `?tags=slug1&exclude=cw-slug` additive with existing params; pagination logic must carry `exclude=` params alongside `tags=` params. No defaults changed — this is a pure opt-in filter. A "power search" mode exposing exclude for all tag categories can be added later without backend changes.
...
- **Suggested tags not appearing in mod queue.** PR #72 intended to surface `tags.status='pending'` chips with a ⚠ badge in queue cards — verified not working live 2026-05-25. Investigate: `admin_poem_queue` query (does it include `t.status AS tag_status` in the join?), `sub.pending_tags` population, and template condition `tag.tag_status == 'pending'`. Fix before next browse/filter work.
