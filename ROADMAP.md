...
- **1.12 Three-mode display system** — spec locked 2026-05-23 (`sessions/2026-05-23-phase-1.12-spec.md`); build pending. Replaces binary plain/pasture with three descriptively-named modes:
  ...
  - **Prototype-first (commit only after Clover reviews rendered options):** Fancy outline color + non-horse de-emphasis; Reader text styling + woodprint borders; Fancy mobile (floating text on narrow viewport); how horse-collection pages read in Reader mode.
  ...
  - **Build order:** 1) infrastructure (resolution, vocabulary, body class, motion CSS, dead-CSS removal) → 2) picker → 3) Plain skin → 4) Fancy skin (static) ⟂ prototype gate → 5) Reader skin ⟂ prototype gate → 6) Fancy mobile ⟂ prototype gate. Steps 1–3 committable without the styling gate. `[opus · high]`
