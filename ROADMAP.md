...
- **Horse chip context menu in collection views** (Clover, 2026-05-23) — on `/me/pasture` and `/me/saved-horses`, horse chips should have a popup context menu with actions: (1) **Send to drafts** — add this horse to a draft stable; (2) **Remove from pasture** — explicit remove (pasture view only); (3) **Save / Unsave** — toggle the blue-ribbon save state. Extends the 1.7 horse popover model but scoped to the list-view collection pages rather than in-poem popovers. Slot alongside or after the style pass.
