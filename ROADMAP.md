...
- **1.27 Save draft from poem builder** (2026-05-19) — Save Draft button + modal; draft-centric stable model (stable belongs to draft, not global pool); SQLite-backed drafts with stable_json + full metadata; `/me/drafts` live page; horse popover on poem permalink gains "Add to draft stable" picker (lazy-fetch, 1-draft direct / 2+-draft picker); anon `horse-draft` localStorage shape with migration from `horse-stable`; `syncLocalToAccount` saves anon draft to SQLite on first login. Removes temporary `stable_horses` table + `/me/stable/*` routes from PR #37.
- **1.28 Draft polish: popover quick-create + editor auto-save redesign** ✅ (2026-05-20) — Popover: 0-draft → inline quick-create form; 1+-draft → list + "Add to new draft ▸" expander; both call `POST /me/draft/create`. Editor: page-load picker when 1+ drafts (server-rendered, no round trip); auto-save on add/remove (immediate), drag reposition (7s debounced); "Currently editing [NAME] [Change Draft ▾]" strip; renamed buttons (Clear Stable / Clear Poem / Edit Details / Post Poem); clear dialogs with destination options (pasture / set loose / send to stable). New backend: `GET /me/draft/get`, `POST /me/draft/create`, updated `GET /me/draft/list`. Draft auto-name changed to "untitled". PR #43. **Testing holds:** Clover to verify all 10 holds from `spec/phase-1.28.md` on live site before 1.29 starts.
