...
- **Horse chip context menu in collection views** (Clover, 2026-05-23) — on `/me/pasture` and `/me/saved-horses`, horse chips should have a popup context menu with actions: (1) **Send to drafts** — add this horse to a draft stable; (2) **Remove from pasture** — explicit remove, shown **only when in the pasture view** (context-sensitive); (3) **Save / Unsave** — toggle the blue-ribbon save state, available everywhere but more prominently placed on the saved-horses page. Extends the 1.7 horse popover model but scoped to collection pages. Slot alongside or after the style pass.
...
- **Admin poem hide / delete** `[sonnet · low]` — admin action to hide a published poem (`poems.status = 'hidden'`) or hard-delete it (CASCADE removes poem_tags, horse_occurrences, saved_poems, submissions). Natural home: the poem permalink (admin toolbar visible to admin users only) and the queue card. A hidden poem should be invisible to the public and excluded from feeds/search but recoverable by admin. Hard delete is permanent. Note: `horses from published poems auto-added to pasture` (backlog) depends on approve/delete events — design that hook here too. Pairs with `admin account actions` below.
- **Admin account actions** `[sonnet · low]` — admin tools on `/admin/user/<id>` (built in 1.13.1): disable/suspend account (block sign-in and queue all future submissions regardless of trust score) and delete account (wipes user row; poems orphan to anonymous via `ON DELETE SET NULL`). Suspension needs a `suspended_at` timestamp or an `active` flag on users. Build alongside or just after `admin poem hide / delete`.
