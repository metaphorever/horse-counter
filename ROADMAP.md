...
- **Collection pages render and behave fully as poems** (Clover, 2026-05-23; expanded 2026-05-24) — single rolled-up effort covering every "list of horses" surface (`/me/pasture`, `/me/saved-horses`, and any future horse-list page). Goal: each page should *be* a dynamically composed poem — same rendering, same view-mode formatting, same interactions — with no bespoke collection treatment. Three threads to fold in:
  1. **Unified chip tap menu** — horse chips get the **same chip menu** used for a horse inside a poem in the editor (`openChipMenu` in poetry.html), not a bespoke per-page popover or the current special-cased `.collection-chip-remove` hover button. Actions: (a) **Send to drafts** — add this horse to a draft stable; (b) **Remove from pasture** — explicit remove, shown **only in the pasture view** (context-sensitive); (c) **Save / Unsave** — toggle blue-ribbon state, available everywhere, more prominent on saved-horses. Extends the 1.7 horse popover model.
  2. **Full view-mode formatting parity** — collection pages must follow the active display mode exactly like a poem permalink does (Fancy / Plain / Reader). **Bug (Clover, 2026-05-24):** in **Plain** mode, `/me/pasture` and `/me/saved-horses` are missing the cream "note" background behind their content that other plain-mode poem surfaces have. Reconcile against how poem permalinks render in Plain mode so they match. (Note: the 1.12 step-4 review logged "Plain note background absence is expected behavior" for a different surface — confirm which rule actually applies here before implementing.)
  3. **Collections-are-poems principle** — this is the concrete realization of the existing principle (memory `[[collections_are_poems]]` + ROADMAP §design). Slot alongside or after the style pass. Not urgent (Clover, 2026-05-24).
