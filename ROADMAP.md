...
- **1.12 Three-mode display system** — spec locked 2026-05-23 (`sessions/2026-05-23-phase-1.12-spec.md`); **build steps 1–3 (infrastructure + picker + Plain skin) shipped + verified live on `poet.horse` 2026-05-23 (`sessions/2026-05-23-phase-1.12-foundation.md`, picker try-on refinement in `…-picker-tryon.md`). Architecture accepted by Clover; remaining nitpicks are styling (deferred to the style-pass session). Prototype-gated steps 4–6 (Fancy enhancements, Reader skin, Fancy mobile) remain.** Replaces binary plain/pasture with three descriptively-named modes:
  ...
  - **Build order:** ✅ 1) infrastructure (resolution, vocabulary, body class, motion CSS, dead-CSS removal) → ✅ 2) picker → ✅ 3) Plain skin → 4) Fancy skin (static) ⟂ prototype gate → 5) Reader skin ⟂ prototype gate → 6) Fancy mobile ⟂ prototype gate. Steps 1–3 shipped + verified live 2026-05-23 (committable without the styling gate); 4–6 await Clover's prototype review. `[opus · high]`
