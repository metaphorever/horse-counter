...
Archived 2026-05-17. Original project documentation before adopting the claude-project-template workflow.
Full git history available via `git log`.


---

# Original ROADMAP.md (at HEAD ebed39e)
...
- 1.5 Poem permalink renderer + Open Graph ✅ — `/p/<short_code>` now renders the full poem page: **title that folds in attribution** ("Title based on Source", with Source linked when `inspired_by_url` is set, italic `<cite>` otherwise; the "based on Source" subtitle reads as part of the H1, smaller and italic), then a courier-mono attribution line with linked display name + published date, the poem body with horse-name links, tags **grouped by category** (with content-warning visual treatment for that category), and a footer with horse/word counts + the short code. Attribution is treated as a special flag, **not** a tag — the page title makes the relationship visible (browser tab, og:title, social previews). The plain/pasture **view-mode toggle** is wired (button + `data-view-mode` attribute on the article + localStorage persistence) so Phase 1.6 can plug in pasture-mode CSS without touching this scaffold. Open Graph tags emitted in `<head>`: `og:title` (includes "based on"), `og:description` (first non-empty line's horse list + total horse count), `og:url`, `og:type=article`, `og:site_name=poet.horse`, `twitter:card=summary`, plus `article:published_time` when present. Unpublished poems are visible to the author and admins only (soft secret — short codes are long enough that guessing them is impractical, the worst case is a draft URL being shared). Templates renamed `poem_stub.html` → `poem.html` to match the route's purpose. Acceptance: pasting a published poem URL in Slack/Discord/iMessage will show a rich preview with the title and a horse-list description; the static-OG-card upgrade is deferred to the Phase 2.3 image-card export.
- 1.6 Two-mode poem renderer (plain / pasture) ✅ — `.poem-view` flips between **plain** (the existing minimal styling from 1.5) and **pasture** (grass-tile background, decorated horse chips with legs/tail/tilt, famous-horse shimmer) via a single `data-view-mode` attribute. Pasture is the documented permalink default; resolution order at boot is server pref (`preferences_json.poem_view_mode` for signed-in users, surfaced via `data-server-mode`) → localStorage → `prefers-reduced-motion` → pasture. Toggle button persists to localStorage and, for signed-in users, POSTs `{poem_view_mode}` to a new **`/me/preferences`** endpoint (allow-listed key validators, never a free-form k/v store). Chip styling ported from `tumblr-theme.html` — same eight coat-class buckets, same `::before` head + leg gradients, same `::after` tail, `.rev` parity flip — but anchored on a `:root` palette of coat-bg/coat-fg variables so the editor and future surfaces can reuse it without copy-pasting hex. `prefers-reduced-motion` zeroes the shimmer and chip tilt but keeps the decoration. Server-side enrichment in `poem_permalink()` adds `coat` / `rev` / `is_famous` to each horse dict (no client JS for coat hashing); a new public `matcher.horse_appearance(name)` wraps `_tile_appearance` for non-rendering callers. Plain-mode HTML is untouched, so screen-reader / RSS / preview-card paths see the same semantic chip tree as before. **Note:** per the post-1.2 aesthetic correction, decorated chips are the *intended* default — pasture mode is the canonical poem-viewing experience, not a fancy opt-in.
- 1.3 Tag taxonomy + selection UI ✅ — four MVP tag categories seeded (`Poem Type` single_select, `Theme` multi_select, `Linguistic Features` multi_select, `Content Warnings` content_warning) with 41 baseline tags. (An "After / Translation" tag was briefly seeded and pulled — see the title-fold note in 1.5; attribution is a flag derived from `poems.inspired_by_text`, not a tag.) Publish modal expanded: optional **After ___** attribution pair (`inspired_by_text` + `inspired_by_url` on `poems`) for crediting source works, plus a per-category section with radio/checkbox chips and a "+ Suggest a new ___ tag" button that submits to a `/tags/suggest` endpoint as `status='pending'` (scoped to the category). Submitter-applied tags land on `poem_tags` with `status='pending'` for 1.13's admin review. Server-side validation drops unknown / non-active tag IDs and dedupes single-select selections. Trust scaffold added: `users.trust_level` column (`pending` default, acted on in 1.13) + empty `data/flagged_horse_names.json`. Idempotent migration + seed runs on every `init_db`. The pending-tag "+ Suggest" UI currently uses a `prompt()` for the input — fine for MVP, may upgrade to inline input later. Verified locally end-to-end: schema migrates clean, tag selection radios/checkboxes behave per category behavior, single-select dedup confirmed both client and server, /tags/suggest handles happy path / duplicate (409) / empty (400), poem insert carries through inspired_by + tag_ids.
...
- Soft-launch list is now mostly the page-side work: **1.7** (horse popover — reuse the chip-menu component from 1.2 against the freshly decorated pasture-mode chips), **1.8** (Featured / Browse / Random), **1.9–1.12** (publish UX polish + reader-mode), **1.13** (admin moderation queue + tag review using the `pending` rows 1.3 wired up).
...
- **`/browse`** — paginated feed (50/page) with:
  ...
  - **Filters (combinable, as chips):** tag (any category), poem type, theme, linguistic feature, author/poet, contains horse (autocomplete dictionary lookup), **time-of-day band** (e.g. "12am–6am", with custom range picker), date range, **"Based on other works"** (filters where `poems.inspired_by_text` is non-empty — the attribution flag from 1.3, not a tag)
  - **Tag picker affordance (forward-looking):** once real usage data exists, surface only the most-used tags inline and collapse the rest under a "more tags" disclosure. Goal: common choices stay easy, less common existing tags stay findable, fewer accidental duplicate suggestions. Computation is straightforward — count of `poem_tags` rows per tag, top N inline. Holding until there's enough traffic to make the cut meaningful.
...
# Original TODO.md (at HEAD ebed39e)
