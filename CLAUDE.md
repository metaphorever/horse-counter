# poet.horse — Claude Code Instructions
...
- Spec `spec/phase-2.7-image-card-export.md`. Every published poem gets a Fancy-view card, rendered once at publish by headless Chromium, stored as static files; **Bluesky** crossposts carry the image instead of a text sample; poems gain a **download** link to a print-quality PNG; card doubles as the site's first `og:image`.
- **Tumblr is OUT of 2.7** (Clover) — it keeps its existing 2.2/2.3 **text** crosspost path, untouched. Removes the phase's riskiest unknown (unverified `data64`-over-JSON photo posts + a multipart contingency). Tumblr images = clean follow-up once the render subsystem is proven.
- **Backfill is load-bearing, not future work** (Clover caught this) — rendering at publish means every pre-2.7 poem has no card. Same selector serves backfill and post-art-change regen: `card_version IS NULL OR card_version < CARD_ART_VERSION`. Bulk goes through **`tools.backfill_cards` CLI**, not an admin request — ~150 poems × ~2–3s = 5–8 min, which Apache/gunicorn time out. Admin button does bounded batches of 10.
